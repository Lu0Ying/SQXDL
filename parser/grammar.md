...
```
statement      -> create_stmt | insert_stmt | select_stmt | update_stmt | delete_stmt | show_stmt | drop_table_stmt
...
（各语句以互不相同的首关键字区分：`CREATE` / `INSERT` / `SELECT` / `UPDATE` / `DELETE` / `SHOW` / `DROP`；
